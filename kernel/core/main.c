...
/* Generated by tools/bin2c.py from the initial ramdisk, and empty when there
 * is no ramdisk to carry. */
extern const unsigned char rk_builtin_initrd[];
extern const unsigned long rk_builtin_initrd_len;
...
static void mount_filesystems(struct boot_info *bi)
{
	...
	/* A module from the bootloader wins: it can be replaced without
	 * rebuilding the kernel. Failing that, the image carries its own copy,
	 * which is what gives a machine whose firmware cannot pass a module -
	 * QEMU's ARM and RISC-V virt machines among them - the same /boot as
	 * everything else. */
	...
	} else if (rk_builtin_initrd_len) {
		rk_vfs_mkdir(NULL, "/boot", 0755);
		rk_initrd_mount_mem(rk_builtin_initrd, rk_builtin_initrd_len, "/boot");
	}
}
